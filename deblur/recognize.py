...
if __name__ == "__main__":
    ...
    for kk in range(10):
        k=kk+90
        InputFile = './result/truth_Ver%d.png'%kk
        if set=='V':
            if k<99:
                trueText = './step%d/Verdana/CAM01/focusStep_%d_verdanaRef_size_30_sample_00%d.txt'%(z,z,k+1)
            else:
                trueText = './step%d/Verdana/CAM01/focusStep_%d_verdanaRef_size_30_sample_0%d.txt'%(z,z,k+1)
        if set=='T':
            if k<99:
                trueText = './step%d/Times/CAM01/focusStep_%d_timesR_size_30_sample_00%d.txt'%(z,z,k+1)
            else:
                trueText = './step%d/Times/CAM01/focusStep_%d_timesR_size_30_sample_0%d.txt'%(z,z,k+1)

        score = score+evaluateImage(InputFile, trueText)
        print(score/10)
